...
def evaluate(repo_path, output_tag):
    ...
    print("="*60)
    print(f"[1. 符号准确率 (Symbol Precision)]")
    print(f"  - gossiphs 发现符号数: {len(gossiphs_normalized)}")
    print(f"  - 准确率 (Precision):   {sym_precision:.2%}")
    
    print(f"\n[2. 文件层级关联 (File-to-File Level)]")
    print(f"  - 基准链路数 (SCIP):   {len(scip_links_set)}")
    print(f"  - gossiphs 链路数:     {len(gossiphs_links_set)}")
    print(f"  - 准确率 (Precision):   {link_precision:.2%}")
    print(f"  - 召回率 (Recall):      {link_recall:.2%}")
    print(f"  - 额外逻辑关联 (Bonus): {len(gossiphs_links_set - scip_links_set)}")
    print("\n" + "="*60)
